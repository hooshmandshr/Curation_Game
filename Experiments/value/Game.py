...
class Game:
	# Size:  size of the graph (number of nodes), 
	# sigma: fraction of the nodes that are contents 
	# pi:    fraction of the nodes that are publishers 
	# rho:   fraction of the nodes that are readers 
	# lamda: amount of contents that every publisher shares (fraction of the entire contents)
	def __init__(self, Size, density, lamda, sigma, pi, rho=0, distribution=True):
		...
		self.Attention = lamda 	

		self.Size = Size

		self.Fractions = {}
		self.Fractions['content'] = sigma
		self.Fractions['publisher'] = pi	
		self.Fractions['reader'] = 1 - sigma - pi

		if distribution:
			
			self.ClassDistribution['content'] = sigma 
			self.ClassDistribution['publisher'] = pi + sigma 
			self.ClassDistribution['reader'] = 1
	...
	def initialize(self):
		...
		self.Attention = int(len(self.contents())*self.Attention)
